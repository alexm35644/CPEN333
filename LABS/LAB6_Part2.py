    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        if chopstick[leftChopstick].acquire(block=False):
            if chopstick[rightChopstick].acquire(block=False):
                eatForAWhile()  # eats only if both chopsticks are acquired 
                print(f"DEBUG: philosopher{id} has chopstick {leftChopstick} and chopstick {rightChopstick}")
            else: 
                print(f"DEBUG: philosopher{id} failed to acquire both chopsticks. Releasing chopstick{leftChopstick}")
